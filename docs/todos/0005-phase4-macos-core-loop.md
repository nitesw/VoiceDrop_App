...
- [ ] Persistent *Menu Bar Icon* using `NSStatusItem`, as a monochrome template image (auto-adapts to light/dark menu bar, per [VISUAL_STYLE.md](../design/VISUAL_STYLE.md)) — glyph source is `assets/VoiceDrop.svg`, pre-converted to `macos/Sources/VoiceDrop/Resources/MenuBarIcon.png`/`@2x`/`@3x` (staged, not yet wired). **Known blocker to resolve first**: SwiftPM's generated `Bundle.module` accessor expects this resource bundle directly at the `.app` root, but placing it there breaks `codesign --verify --deep --strict` ("code has no resources but signature indicates they must be present") — tried and confirmed broken. Needs a real fix (e.g. relocate into `Contents/Resources/` and adjust how the asset is loaded, bypassing `Bundle.module`, or load the PNGs directly by absolute path relative to the executable) before this can actually load at runtime
